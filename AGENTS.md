...
A Python CLI tool (`fantastical`) providing composable access to Fantastical calendar data on macOS, with an MCP server for AI assistant integration. Installed via `uv sync`, run via `uv run fantastical <command>`.
...
│   ├── shortcuts.py       # `shortcuts run` CLI wrapper (event queries, attendees, setup checks)
...
└── server.py              # FastMCP server — compact output, event/attendee caching
```
...
4. **`server.py`** calls only `api.py`. Returns compact tab-separated text for list/search tools (with in-memory event/attendee caching), JSON for create. See `server.py` for per-tool response formats.
...
**Find Attendees flow:** Same input parsing as Find Events → CalendarItemQuery → Get Item 1 (first matching event) → **If "has any value"** → FKRGetAttendeesFromEventIntent → Repeat Each (over attendees) → Text(displayString`\x1f`email`\x1e`) → End Repeat → Text wrap → Otherwise (empty output) → End If → Output. The If guard prevents `FKRGetAttendeesFromEventIntent` from receiving a nil `calendarItem`, which would cause Fantastical to show a "Select an event" modal dialog. Uses Level 4 pattern (single-level Repeat Each) because nested Repeat Each fails for inner property access (see `_experiments/22_attendees_level1.py`). Called on demand from `get_event_details` with lazy caching.
...
- Caller passes exact date range. No hard cap on the caller side — `get_events()` in `shortcuts.py` automatically batches into `_CHUNK_DAYS`-day windows (currently 4 days) to work around a macOS Shortcuts runtime bug where `FKRGetAttendeesFromEventIntent` inside a Repeat Each loop creates a duplicate action instance after ~260 iterations, showing a blocking "Select an event" modal. MCP tool descriptions still guide agents to start with 2-week chunks and widen if the calendar is sparse.
...
Each shortcut has its own CalendarItemQuery which triggers a separate **privacy dialog** ("smart prompt") on first run after import. The `setup` command warms up **both** Find Events and Find Attendees to trigger these grants during setup so users don't see them later. If a new shortcut is added, it MUST be warmed up in the setup test section of `cli.py` — otherwise the privacy dialog will appear on first real use. If the dialog doesn't appear during setup, try restarting Shortcuts.app (`killall Shortcuts && open -a Shortcuts`).
...
- The MCP server (`server.py`) uses FastMCP. Each `@mcp.tool()` function calls the corresponding `api.*` function. List/search tools return compact tab-separated text with event caching; `create_event` returns JSON; `get_event_details` returns key-value text with lazily-fetched attendees.
...
`docs/resolved_tickets/` contains archived resolved tickets with original context, changes, and reasoning. One file per ticket, prefixed with date for chronological ordering. Useful for understanding why things are the way they are.
